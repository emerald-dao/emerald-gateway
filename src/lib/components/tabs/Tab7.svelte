<script>
    ...
    import CollectionComponent from "../../components/tabs/tab3/CollectionComponent.svelte";
    import {
        tab1data,
        tokens,
        collections,
...
<h3>Tokens</h3>
<div
    style="display: flex; flex-direction:{mobile ? 'column' : 'row'}"
    class="mt-1"
>
    {#each Tokens as { imgUrl, label, amount, selected, id }}
        {#if selected}
            <div
                style="--width:{mobile ? '60%' : '30%'}; --margin-left:{id ===
                    0 && !mobile
                    ? '0rem'
                    : '0.8rem'} --margin-top:{mobile ? '0.6rem' : '0rem'}"
