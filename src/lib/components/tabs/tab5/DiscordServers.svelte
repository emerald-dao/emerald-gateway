<script>
    import { discordVerif } from "$lib/stores";
    import Icon from "@iconify/svelte";

    function removeServer(id) {
        $discordVerif.servers = $discordVerif.servers.filter(
            (value, index, arr) => {
                if (value.id != id) {
                    return value;
                }
...
<ul class="mt-2">
    {#each $discordVerif.servers as server, i}
        <li class="server-container">
            ...
            <div
                class="icon-container flex-align"
